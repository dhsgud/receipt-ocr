...
class ReceiptOCR:
    ...
    def process_image(self, image_data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            prompt = """이 영수증/급여명세서 이미지를 분석하여 JSON 형식으로 정리해주세요.

다음 JSON 형식으로 응답해주세요:
{
    "store_name": "상호명 또는 회사명",
    "date": "YYYY-MM-DD",
...
⚠️ 수입/지출 구분 규칙 (중요!):
- 급여명세서, 월급, 급여, 봉급, 상여금, 보너스 → is_income: true
- 배당금, 이자, 환급금, 정산금, 캐시백 → is_income: true
- 입금, 지급받음, 송금받음, 이체받음 → is_income: true
- 구매/결제 영수증, 지출 내역 → is_income: false

⚠️ 카테고리 선택 규칙:
[지출 카테고리] (is_income: false인 경우)
- 식비: 음식점, 카페, 편의점, 마트 식품
- 교통: 주유소, 대중교통, 택시, 주차
- 쇼핑: 의류, 전자제품, 생활용품
- 의료: 병원, 약국
...
⚠️ 품목명 처리 규칙:
- 잘린 품목명은 전체 이름으로 복원 (예: "신라면멀" → "신라면멀티팩")
- 추측 어려우면 잘린 그대로 + "?" 표시
